...
def plot_stages(autoencoder, data, save=False, plot=True, path=None, get_frame=False):
    ...
    fig.subplot(2, 2, 3)
    fig.title('decoded')
    fig.imshow(decoded[0].cpu().numpy())
    fig.axis('off')
    fig.subplot(2, 2, 4)
    fig.title('Reconstruction error')
    image = cv2.cvtColor(data.cpu().permute(1, 2, 0).numpy(), cv2.COLOR_RGB2GRAY)
    reconstructed = cv2.cvtColor(decoded[0].cpu().permute(1, 2, 0).numpy(), cv2.COLOR_RGB2GRAY)
    fig.imshow(image - reconstructed, vmin=0, vmax=1, cmap="viridis")
    fig.axis('off')
